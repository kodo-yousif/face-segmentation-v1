...
def load_dataset(path):
    ...
    indices = np.random.choice(len(train_x), samples, replace=False)
    train_x = [train_x[i] for i in indices]
    ...
    indices = np.random.choice(len(valid_x), samples, replace=False)
    valid_x = [valid_x[i] for i in indices]
    valid_y = [valid_y[i] for i in indices]


    test_x = sorted(glob(os.path.join(path, "test", "images", "*.jpg")))
    test_y = sorted(glob(os.path.join(path, "test", "labels", "*.png")))

    indices = np.random.choice(len(test_x), samples, replace=False)
    test_x = [test_x[i] for i in indices]
    test_y = [test_y[i] for i in indices]
...
if __name__ == "__main__":
    ...
    """ Paths """
    dataset_path = "/Users/kodo/Projects/Human-Face-Segmentation-in-TensorFlow/LaPa"
